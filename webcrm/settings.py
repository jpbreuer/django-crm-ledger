...
# Add django-ledger package on path for integration
sys.path.insert(0, str(BASE_DIR / 'django-ledger'))
...
try:
    import MySQLdb  # check if mysqlclient is installed
except ImportError:
    # Fallback to SQLite if MySQLdb is not installed
    DATABASES = {
        'default': {
...
APP_ON_INDEX_PAGE = [
    'tasks', 'crm', 'analytics', 'django_ledger',
...
MODEL_ON_INDEX_PAGE = {
    ...
    'analytics': {
        'app_model_list': [
            'IncomeStat', 'RequestStat', 'django_ledger'
        ]
    },
    'massmail': {
        'app_model_list': [
...
# List of applications on the admin index page.
ADMIN_APP_ON_INDEX_PAGE = [
    'analytics', 'auth', 'chat', 'common', 'crm', 'django-ledger', 'help', 'massmail', 'settings', 'sites', 'tasks'
]

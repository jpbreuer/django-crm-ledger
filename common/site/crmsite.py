...
def index(self, request, extra_context=None):
    ...
    # Filter apps for admin index page if configured
    if hasattr(settings, 'ADMIN_APP_ON_INDEX_PAGE'):
        allowed = settings.ADMIN_APP_ON_INDEX_PAGE
        app_list = [app for app in app_list if app.get('app_label') in allowed]
    # Add view_only flag and add_url to models for admin site index
    for app in app_list:
        for model in app.get('models', []):
            perms = model.get('perms', {})
            # Flag models without add/change/delete permissions as view_only
            model['view_only'] = not (perms.get('add') or perms.get('change') or perms.get('delete'))
            # Provide add_url if user has add permission
            if perms.get('add'):
                try:
                    model['add_url'] = reverse(
                        f'admin:{app["app_label"]}_{model["object_name"].lower()}_add'
                    )
                except NoReverseMatch:
                    model['add_url'] = None
            else:
...
class BaseSite(admin.AdminSite):
    ...
    def index(self, request, extra_context=None):
        ...
        # Add view_only flag to models for CRM front site index
        for app in app_list:
            for model in app.get('models', []):
                perms = model.get('perms', {})
                model['view_only'] = not (perms.get('add') or perms.get('change') or perms.get('delete'))
